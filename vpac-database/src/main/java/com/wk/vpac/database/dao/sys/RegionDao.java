...
public interface RegionDao extends GenericJpaDao<Region, String> {
   ...
   * @return roots
   */
  @Query("from Region where parentId='" + ROOT_PARENTID + "' and status = :status order by orderNo asc")
  List<Region> findRoots(@Param("status") Integer status);
   ...
   * @return
   */
  @Query("from Region where parentId=:parentId and status = :status order by orderNo asc")
  List<Region> findByParentId(@Param("parentId") String parentId, @Param("status") Integer status);
  ...
  List<RegionCountDto> findRootsWithUserCount();
  ...
  List<RegionCountDto> findByParentIdWithMemberCount(@Param("parentId") String parentId);
  ...
  List<RegionCountDto> findByParentIdWithNormalUserCount(@Param("userType") int userType, @Param("parentId") String parentId);

  @Query(value = "select id,name,parent_id parentId,level from base_region order by id", nativeQuery = true)
  List<Map> allBycitySelect();
}
