...
public class SysAppInfoService extends AbstractJpaService<SysAppInfo, String, SysAppInfoDao> {
  public static final Set<String> ILLEGAL_URI = Sets.newHashSet("*", "**", "/", "/*", "/**");
  ...
  private void checkDirectory(SysAppInfo appInfo){
    if(appInfo.getType() != 0){
      if(StringUtils.isBlank(appInfo.getAuthUrlPrefix())){
        throw new IllegalArgumentException("授权uri，不能为空 !");
      }
      if(appInfo.getType() == 1){
        if(StringUtils.isBlank(appInfo.getTargetUrl()) || "/".equals(appInfo.getTargetUrl())){
          throw new IllegalArgumentException("跳转路径，不能为空且不等于“/” !");
        }
      }
      if (appInfo.getType() == 2){
        appInfo.setTargetUrl(null);
      }
      String[] split = StringUtils.split(appInfo.getAuthUrlPrefix(), ",");
      Set<String> url = Sets.newHashSet();
      for (String u : split) {
        String path = u.substring(u.indexOf(":") + 1);
        if(StringUtils.isNotBlank(path) && !path.contains(":") && !ILLEGAL_URI.contains(path) ){
          url.add(u);
        }
      }
      addTargetUrlAuth(url, appInfo.getTargetUrl());
      Assert.isTrue(!url.isEmpty(), "授权uri，效验未通过 !");
      appInfo.setAuthUrlPrefix(StringUtils.join(url, ","));
    }
    else{
      appInfo.setTargetUrl(null);
  ...
  private void addTargetUrlAuth(Set<String> authUrl, String targetUrl){
    if(StringUtils.isNotBlank(targetUrl)){
      String targetAll = "*:" + targetUrl;
      String targetGet = "GET:" + targetUrl;
      boolean exist = false;
      for (String url : authUrl) {
        if(AdminAuth.MATCHER.match(url, targetAll) || AdminAuth.MATCHER.match(url, targetGet)){
          exist = true;
          break;
        }
      }
      if(!exist){
        authUrl.add(targetGet);
      }
    }
  }
  ...
  public void delete(String id) throws IllegalArgumentException{
    ...
    Assert.isTrue(appInfo.getLeftVal() > 1, "can not delete root !");
