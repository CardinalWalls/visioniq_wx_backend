...
import com.wk.vpac.common.dto.admin.SysAppInfoDto;
...
public class SysMemberService extends AbstractJpaService<SysMember, String, SysMemberDao> {
  public static final String APP_MENU_SUFFIX = "_MENU";
   ...
   * @return
   */
  public List<TreeNodeData<SysMember>> loadTreeNodes(TreeParam treeParam, String treeKind) {
    return loadTreeNodes(treeParam, new MemberNodeBuilder(getDao()), treeKind, null, CountType.NONE);
  }
   ...
   * @return
   */
  public List<TreeNodeData<SysMember>> loadTreeNodes(TreeParam treeParam, BaseNodeBuilder<SysMember> nodeBuilder,
    ...
    addNodes(nodes, isRoot, treeParam, nodeBuilder, treeKind, selectIds, countType);
    return nodes;
  }
   ...
   *
   * @param treeParam
   * @param BaseNodeBuilder
   * @param treeKind
   * @param selectIds
   *
   * @return
   */
  public List<TreeNodeData<SysMember>> loadAllNodes(TreeParam treeParam, BaseNodeBuilder<SysMember> BaseNodeBuilder,
    ...
    if (!all.isEmpty()) {
      ...
      TreeNodeData<SysMember> root = BaseNodeBuilder.buildNode(all.get(0), selectIds, treeParam);
      buildAllNodes(root, treeParam, BaseNodeBuilder, parentIdMap, selectIds);
      nodes.add(root);
    }
    return nodes;
  }

  private void buildAllNodes(TreeNodeData<SysMember> node, TreeParam treeParam, BaseNodeBuilder<SysMember> BaseNodeBuilder,
    ...
    if (children != null && !children.isEmpty()) {
      for (SysMember child : children) {
        TreeNodeData<SysMember> childNode = BaseNodeBuilder.buildNode(child, selectIds, treeParam);
        childrenNodes.add(childNode);
        buildAllNodes(childNode, treeParam, BaseNodeBuilder, parentIdMap, selectIds);
      }
    }
  ...
  public UserMemberToken updateBaseInfoAndTokenObj(Map<String, String> param) {
    ...
    SystemLogManager.getInstance().addLog(memberToken);
    return memberToken;
  }
...
//                      .setParameter("mId", member.getId()).executeUpdate();
  ...
  public UserMemberToken login(@NonNull String username, @NonNull String password, String loginIp) {
    ...
    if (exists.getStatus() == Valid.FALSE.getVal()) {
    ...
    SystemLogManager.getInstance().addLog(userMemberToken);
    ...
    if (!CollectionUtils.isEmpty(appInfoList)) {
      ...
      Map<String, String> indexUri = Maps.newHashMap();
      Set<String> equalsUri = Sets.newHashSet();
      Set<String> patternUri = Sets.newHashSet();
      ...
      for (SysAppInfoDto dto : appInfoList) {
        ...
        if(StringUtils.isNotBlank(dto.getTargetUrl())){
          indexUri.put(dto.getTargetUrl(), dto.getName());
        }
        addAuthUri(dto.getAuthUrlPrefix(), dto.getName(), equalsUri, patternUri);
      }
      for (SysAppInfoDto root : roots) {
        buildAuthAppInfoDtoMenuTree(root, parentIdMap);
      }
      ...
      adminAuth.setIndexUri(indexUri);
      adminAuth.setEqualsUri(equalsUri);
      adminAuth.setPatternUri(patternUri);
      Cache cache = cacheManager.getCache(CacheName.ADMIN_AUTH_INFO);
      cache.put(exists.getId(), adminAuth);
      putAppInfoMenuJson(exists.getId(), roots);
    }
  ...
  public AdminAuth getCurrentAdminAuth() {
    ...
    if (auth != null) {
      ...
      cache.expire(auth.getTokenObjId() + APP_MENU_SUFFIX, cache.getDefaultExpiresSecond(), TimeUnit.SECONDS);
    }
    return auth;
  }
  ...
  public void putAppInfoMenuJson(String memberId, List<SysAppInfoDto> appInfoList){
    if(!CollectionUtils.isEmpty(appInfoList) && memberId != null){
      Cache cache = cacheManager.getCache(CacheName.ADMIN_AUTH_INFO);
      cache.put(memberId + APP_MENU_SUFFIX, appInfoList);
    }
  }

  @SuppressWarnings("unchecked")
  public List<SysAppInfoDto> loadAppInfoMenuJson(String memberId){
    if(memberId != null){
      String key = memberId + APP_MENU_SUFFIX;
      Cache cache = cacheManager.getCache(CacheName.ADMIN_AUTH_INFO);
      ArrayList arrayList = cache.get(key, ArrayList.class);
      if(arrayList != null){
        cache.expire(key, cache.getDefaultExpiresSecond(), TimeUnit.SECONDS);
        return arrayList;
      }
    }
    return null;
  ...
  private void buildAuthAppInfoDtoMenuTree(SysAppInfoDto node, ListMultimap<String, SysAppInfoDto> parentIdMap) {
    List<SysAppInfoDto> children = Lists.newArrayList(parentIdMap.get(node.getId()));
    if (!CollectionUtils.isEmpty(children)) {
      Iterator<SysAppInfoDto> iterator = children.iterator();
      while (iterator.hasNext()){
        SysAppInfoDto child = iterator.next();
        // 2 = 功能资源
        if(child.getType() == 2){
          iterator.remove();
        }else{
          buildAuthAppInfoDtoMenuTree(child, parentIdMap);
        }
      }
...
//      }
  ...
  private void addAuthUri(String authUri, String appName, Set<String> equalsUri, Set<String> patternUri){
    String[] split = StringUtils.split(authUri, ",");
    if(split != null){
      for (String uri : split) {
        String u = uri.startsWith("*:")?uri.substring(2):uri;
  ...
  private void checkMemberType(SysMember member, String existsAccount) {
    if (MemberType.PERSON.getType() == member.getMemberType()) {
      ...
      Assert.isTrue(PasswordCheckUtil.evalPassword(member.getPwd()), "密码过于简单，请更换密码！");
  ...
  private void addNodes(List<TreeNodeData<SysMember>> nodes, boolean isRoot, TreeParam params,
                        BaseNodeBuilder<SysMember> builder, String treeKind, List<String> selectIds, CountType countType) {
    List<SysMember> list;
    if (isRoot) {
  ...
  private void addChildren(TreeNodeData<SysMember> parentNode, SysMember parent, TreeParam params,
                           BaseNodeBuilder<SysMember> builder, List<String> selectIds) {
    List<TreeNodeData<SysMember>> children = Lists.newArrayList();
  ...
  private boolean buildRoleIdsAndCodes(List<SysRole> roles, Set<String> roleIds, Set<String> roleCodes) {
    ...
    Set<String> superRoleIds = baseAdminProperties.getSuperRoleIds();
    for (SysRole r : roles) {
      roleIds.add(r.getId());
  ...
  public DataPage couponListNormalUser(String userPhone, Pageable pageable) {
    ConditionGroup<UserBaseInfo> build = ConditionGroup.build();
    if(StringUtils.isNotBlank(userPhone)){
      build.addCondition("phone",ConditionEnum.OPERATE_EQUAL,userPhone);
    }
    Page<UserBaseInfo> page = userBaseInfoDao.findAll(build,
      pageable
    );
    return DataPage.from(page);
  }

  public void addTargetPathCookie(HttpServletRequest request, HttpServletResponse response){
    if(request != null && response != null){
      String target = request.getParameter("target");
      if(StringUtils.isNotBlank(target)){
        Cookie cookie = new Cookie("_target_", target);
