    ...
    $.extend($.fn.bootstrapTable.defaults, {
      ...
      onTreeFormatter: function onTreeFormatter(row) {
        var that = this,
          ...
          paddings = [];
        var isParentLast = row._parent && row._parent._last;
        var nonePath = row._parent ? (row._parent._none_path ? row._parent._none_path : "") : "";
        for (var i = 0; i < plevel; i++) {
          var noneClass = nonePath.charAt(i) === "0" ? " none " : "";
          paddings.push('<i class="' + options.treeVerticalcls +noneClass+'"></i>');
        ...
        for (var i = plevel; i < level; i++) {
          if (row._last && i === level - 1) {
            ...
            row._none_path = nonePath + "0";
          } else {
            row._none_path = nonePath + "1";
