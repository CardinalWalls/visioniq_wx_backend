/**
 * api 见https://terryz.oschina.io/selectpage/docs.html
 *
 * @summary     SelectPage
...
;(function ($) {
  ...
  SelectPage.prototype.setElem = function (combo_input) {
    ...
    // set outer box width
    elem.container.css({"min-width": orgWidth});
...
*/
    ...
    if (key) {
      if (typeof p.data === 'object') {
        ...
        if (_paramsFunc && $.isFunction(_paramsFunc)) {
          data = $.extend(data, _paramsFunc.call(self));
        }
        $.ajax({
          dataType: 'json',
          type: 'GET',
  ...
  $.selectPageInit = function (multiple, el, url, eSelect) {
    el.selectPage({
      showField: 'name',
      keyField: 'id',
      data: ctx + url,
      multiple: multiple,
      eSelect: eSelect,
      //ajax请求后服务端返回的数据格式处理, 返回的数据里必须包含list（Array）和totalRow（number|string）两个节点
  ...
  $.selectPage = function (el, url, extOption) {
    el.each(function(){
      var opt = {
