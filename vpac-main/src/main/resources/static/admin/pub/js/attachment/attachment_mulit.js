...
mulit可显示多个图片了，详见commodity/index.js/ftl
...
var attachment = {
  ...
  data: new Map(),
  imgClass: 'img-circle',
  ossurl: ctx+'/api/oss/redirect?file=',//x-oss-process=image/resize,l_100&
  getAtta: function (name) {
    attachment.name = name;
    attachment.pattern = attachment.data[name];
    return this;
  },
  regist: function (name, pattern, imgClass) {
    attachment.name = name;
    attachment.pattern = pattern;
    attachment.data[name] = pattern;
    if(imgClass){
      attachment.imgClass = imgClass;
    }
    //上传文件事件
        ...
        success: function (res) {
          if (res.success) {
            ...
            $("#" + attachment.name + "Form #photos").append(
                '<img class="'+attachment.imgClass+'" style="width: 90px;height: 90px;margin:0 2px 2px 0" src="'+res.data.url+'" ><a href="#" onclick="attachment.deletePre(this, \''+rnd+'\')"><i class="fa fa-trash-o "></i></a></img>'
            );
          }
          else {
  ...
  appendImg: function (id, uri) {
    $("#" + attachment.name + "Form #photos").append(
        '<img class="'+attachment.imgClass+'" style="width: 90px;height: 90px;margin:0 2px 2px 0" src="'+attachment.ossurl+uri+'" ><a href="#" onclick="attachment.deletePre(this, \''+id+'\')"><i class="fa fa-trash-o "></i></a></img>'
    );
  },
  ...
  deletePre: function (e, id) {
    $(e).prev('.'+attachment.imgClass).remove();
    $(e).remove();
    $('#attaDiv_' + id).remove();
  },
  clear: function () {
