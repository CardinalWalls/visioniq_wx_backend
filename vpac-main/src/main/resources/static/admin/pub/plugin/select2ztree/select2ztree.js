 ...
 *
 * @param options object类型的select2配置参数{...,ztree:{setting:{},zNodes:[]},valueField:值属性名称,textField:文本属性名称}
 * @version: v1.0.0
 */
!function ($) {
  'use strict';
  ...
  $.fn.select2.amd.define('select2/data/ztree', ['./base', '../utils', 'jquery'], function (BaseAdapter, Utils, $) {
    ...
    ZtreeSelectAdapter.prototype.select = function (data) {
      ...
      console.log(data);
      this.$element.attr("data-name",$(name).is("span")?data.oldname:name);
      this.$element.attr("data-level", level);
